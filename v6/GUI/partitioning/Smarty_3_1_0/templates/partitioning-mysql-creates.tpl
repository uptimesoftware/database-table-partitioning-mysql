...
<p>Current Date/Time: {$currentDatetime}<br />
Number of Partitions selected: {$num_of_partitions}<br />
Change number of partitions: 

<form method='get'>
<select name='partitions'>
{section name=parts start=min_partitions+2 loop=21 step=1}
{$selected = ($smarty.section.parts.index == $num_of_partitions) ? " selected" : ""}
<option {$selected}>{$smarty.section.parts.index}</option>
{/section}
</select>
...
CREATE TABLE vsync_update_par (
  ...
  KEY VSYNC_UPDATE_VMOID_TIME (vmware_object_id,timestamp)
)
{$partition_timestamp}
