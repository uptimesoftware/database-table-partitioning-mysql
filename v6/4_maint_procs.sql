...
BEGIN
	DECLARE months int;
	SELECT uptime_get_ap_months('vmware') INTO months;
...
DROP PROCEDURE IF EXISTS uptime_manage_vmw_vsync_update $$
CREATE PROCEDURE uptime_manage_vmw_vsync_update ()
BEGIN
	DECLARE months int;
	SELECT uptime_get_ap_months('vmware') INTO months;
	CALL uptime_manage_partitions('vsync_update', months);
END $$
...
DO BEGIN
	...
	CALL uptime_manage_vmw_vsync_update();
END $$
